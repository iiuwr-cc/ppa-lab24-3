...
let string_of_arith_unop = function UNOP_Neg -> "-" | UNOP_Deref -> "*"
...
let prio_of_arith_unop = function UNOP_Neg -> 30 | UNOP_Deref -> 30
let prio_of_bool_unop = function UNOP_Not -> 30

let prio_of_arith_expression = function
  | AEXPR_Int _ | AEXPR_Null | AEXPR_AddressOf _ | AEXPR_Var _ -> 100
...
let rec string_of_arith_expression = function
  ...
  | AEXPR_AddressOf { var; _ } -> Format.sprintf "&%s" (string_of_id var)
  ...
  | AEXPR_UnOp { sub; op } ->
      let my_prio = prio_of_arith_unop op in
      let sub_prio = prio_of_arith_expression sub in
      String.concat ""
        [
          string_of_arith_unop op;
          bracket_if my_prio sub_prio @@ string_of_arith_expression sub;
        ]
...
let rec string_of_lvalue_expression = function
  | LVALUE_Var { var } -> string_of_id var
  | LVALUE_Deref { sub } ->
      Format.sprintf "*%s" (string_of_lvalue_expression sub)

let string_of_seq_statement = function
  | STMT_Skip _ -> "skip"
  | STMT_Malloc { lhs; _ } ->
      Format.sprintf "%s = malloc" (string_of_lvalue_expression lhs)
  | STMT_Goto { target; _ } -> Format.sprintf "goto %s" (string_of_label target)
  | STMT_Assign { lhs; rhs; _ } ->
      Format.sprintf "%s = %s"
        (string_of_lvalue_expression lhs)
        (string_of_arith_expression rhs)
