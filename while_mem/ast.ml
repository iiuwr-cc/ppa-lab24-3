...
type arith_unop = UNOP_Neg | UNOP_Deref
...
let rec _aexprs_of_arith_expression aux = function
  ...
  | AEXPR_AddressOf _ as e -> AExprSet.add e aux
...
let rec _vars_of_arith_expression aux = function
  ...
  | AEXPR_AddressOf { var; _ } -> IdSet.add var aux
...
type lvalue_expression =
  | LVALUE_Var of { var : id }
  | LVALUE_Deref of { sub : lvalue_expression }

let rec _vars_of_lvalue_expression aux = function
  | LVALUE_Var { var } -> IdSet.add var aux
  | LVALUE_Deref { sub } -> _vars_of_lvalue_expression aux sub

let vars_of_lvalue_expression_set x = _vars_of_lvalue_expression IdSet.empty x

type struct_statement =
  | SSTMT_Assign of {
      lhs : lvalue_expression;
      rhs : arith_expression;
      loc : location;
    }
  | SSTMT_If of {
  ...
  | SSTMT_Malloc of { loc : location; lhs : lvalue_expression }
...
type seq_statement =
  ...
  | STMT_Malloc of { loc : location; lhs : lvalue_expression }

let _vars_of_seq_statement aux = function
  | STMT_Assign { lhs; rhs; _ } ->
      let aux = _vars_of_lvalue_expression aux lhs in
      let aux = _vars_of_arith_expression aux rhs in
      aux
  | STMT_IfGoto { cond; _ } ->
  ...
  | STMT_Goto _ -> aux
  | STMT_Skip _ -> aux
  | STMT_Malloc { lhs; _ } -> _vars_of_lvalue_expression aux lhs
