...
module AndersenImpl = struct
  let process source = Utils.parse source |> Utils.unstructurize

  let run source_file =
    let program = process source_file in
    Andersen.analyse ~verbose:false ~print:true program
end
...
module AndersenCommand = struct
  open Cmdliner

  let compile src =
    let _ = AndersenImpl.run src in
    0
  ...
  let cmd =
    let compile_t = Term.(const compile $ source_file) in
    let info = Cmd.info "andersen" ~doc:apk_doc ~version:apk_version in
    Cmd.v info compile_t
...
let command =
  make_switch
    [
      ...
      ("andersen", (module AndersenCommand : SCommand));
    ]
